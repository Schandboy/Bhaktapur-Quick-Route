...
public class Get_koordinat_awal_akhir extends Activity{
	...
	JSONObject jadi_json = new JSONObject();
				//so
						...
						//take node from field early_node and destination in table graph, trs merged; example 1,0 and pounding to array
	...
	public JSONObject Get_simpul(double latx, double lngx, Context context) throws JSONException {
		...
		//accomodate node from field early node and destination in table graph,trs merged;example 1, 0 and put in to array
		...
		for(int j = 0; j < indexBarisYgDikerjakan.size(); j++){
			...
			if(indexBarisYgDikerjakan1.length() == 0){
				//field id again on table graph (special for stringbuilder)
		...
		for(int k = 0; k < cursor.getCount(); k++){
				// variable create search 1 distance in 1 record (1 record content many coordinate)
			// VARIABEL BUAT CARI 1 JARAK DALAM 1 RECORD (1 record isinya banyak koordinat)
			//save it distance user to coordinate node in meter
			...
			//get it coordinate Lat,Lng from field coordinate (3)
			...
			for(int w = 0; w < jArrCoordinates.length(); w++){
				...
				//search distance from position user to coordinate around node (in meter)
			...
			//search weight reply most small
			...
			for(int m = 0; m < jarakUserKeKoordinatSimpul.size(); m++){
				...
				if(jarakUserKeKoordinatSimpul.get(m) <= jarakUserKeKoordinatSimpul.get(0)){
					...
					//				from value reply smallest
					// index array dari value yg terkecil
					index_koordinatSimpul = m;
				}				
			}	

				//field id from table graph
			// field id dari table graph
			int row_id = cursor.getInt(0);
			
			JSONObject list = new JSONObject();					
			//put in index coordinate array, weight smallest and amount coordinate to JSON
		...
		String nodes_json = "";
		//search weight smallest from JSON
		...
		//coordinate reply obtained in early or at the end,then not need add nodes
		...
		}//coordinate that obtained located in the middle 2 node 0-1 (example)
		//Koordinat yang didapat berada ditengah2 simpul 0 - 1 (misal)
		else{
			//create node double;node behind
			...
			//there is node reply double (1,0) and (0,1)
		...
		//so_json.put							titleTable_destinationnode
