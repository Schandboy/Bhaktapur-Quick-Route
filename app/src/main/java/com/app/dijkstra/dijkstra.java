	...
	void jalurTerpendek(String[][] arg_graph, int simpulAwal, int simpulTujuan){
        ...
        if(simpul_maju != simpul_tujuan){
            ...
            for(int perulangan = 0; perulangan < 1; perulangan++){
                ...
                //for get in 1 weight most minimum from every node
                List<Double> perbandinganSemuaBobot = new ArrayList<Double>();
                //register node first that will done to in array
                ...
                //iteration node node that be marked
                ...
                for(int perulanganSimpul = 0; perulanganSimpul < simpulYangDikerjakan.size(); perulanganSimpul++){
                    //count amount line per column node
                    ...
                    int jml_baris_fix = 0;
                        ...
                        for(int min_batas_bris_fix = 0; min_batas_bris_fix < jml_baris_fix; min_batas_bris_fix++){
                            ...
                            //search weight reply not yet done (reply no there is sign->y)
                            if(explode.length == 2){
                                status_baris += 1; // masih ada yg belum ->y
                                                    //still there is reply not yet->y
                                //check node is already be marked what bolom,klo udh means value * no plus again/0
                                //Cek simpul apakah sudah ditandai apa blom, klo udh berarti nilai * tidak ditambah lagi / 0
                                // reply blm be marked, means value * worth value node reply marked
                        ...
                        //if line in column not yet -> y all, then do it if in under this
                        //jika baris di kolom belum ->y semua, maka lakukan if di bawah ini :
                        if(status_baris > 0){
                            //get it weight minimum
                ...
                //end for iteration node
                //get it 1 weight most minimum from node reply be marked
                ...
                //get it index node+ its weight reply original from node reply be marked
